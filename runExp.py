...
def main():
    ...
    f2bridge_cl_tracker = GenerationMetrics(model=model, device=args.device,
                                tokenizer=tokenizer, dataset_name=args.dataset_name,
                                fname=fname+"_forced_2_bridgeCLEmbs_"+args.method,
                            model_args=args,
                                subclass="BRIDGE")
    
    f3bridge_cl_tracker = GenerationMetrics(model=model, device=args.device,
                                tokenizer=tokenizer, dataset_name=args.dataset_name,
                                fname=fname+"_forced_3_bridgeCLEmbs_"+args.method,
                            model_args=args,
                                subclass="BRIDGE")
    ...
    sampled_text = random.sample(range(num_intervals), 5)
    for  num_example in tqdm.tqdm(sampled_text):
        ...
        LABELS = ['TRUE CL', 'BRIDGE CL (DE)', 'EXTREME BRIDGE CL zoom2', 'EXTREME BRIDGE CL zoom3','RANDOM CL']
        ...
        num_sentences = len(torch.unique(true_cl_feats,dim=0)) if not args.split_sentences else int(len(torch.unique(true_cl_feats,dim = 0))/float(args.split_sentences))
        ...
        bridge_force_2 = simulate_brownian_bridge_zoom(
    B_0=true_cl_feats[0], B_T=B_T, num_samples=num_sentences,
    sentence_lengths=end_lengths, zoom = 2)
        bridge_force_2 = torch.tensor(
            bridge_force_2, dtype=true_cl_feats.dtype).to(args.device)
        
        
        bridge_force_3 = simulate_brownian_bridge_zoom(
    B_0=true_cl_feats[0], B_T=B_T, num_samples=num_sentences,
    sentence_lengths=end_lengths, zoom = 3)
        bridge_force_3 = torch.tensor(
            bridge_force_2, dtype=true_cl_feats.dtype).to(args.device)
        ...
        feats = [true_cl_feats, bridge_feats, bridge_force_2, bridge_force_3, random_feats]


        trackers = [gt_cl_tracker, bridge_cl_tracker,  f2bridge_cl_tracker,  f3bridge_cl_tracker,
                    random_cl_tracker
                    ]
